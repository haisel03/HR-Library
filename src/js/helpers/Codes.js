...
const _BARCODE_FORMATS = Object.freeze(["CODE128", "CODE39", "EAN13", "EAN8", "UPC", "ITF14", "MSI", "pharmacode"]);
...
const Codes = {
	...
	barcode(target, value, options = {}) {
		const el = _el(target);
		if (!el) {
			console.warn("[Codes] Elemento no encontrado:", target);
			return false;
		}
		if (value === null || value === undefined || value === "") {
			console.warn("[Codes] Valor de barcode vacío.");
			return false;
		}
	...
	async qrCanvas(target, text, options = {}) {
		const el = _el(target);
		if (!el || !text) {
			console.warn("[Codes] qrCanvas: elemento o texto inválido.");
			return false;
		}
		try {
	...
	async qrImage(target, text, options = {}) {
		const el = _el(target);
		if (!el || !text) {
			console.warn("[Codes] qrImage: elemento o texto inválido.");
			return false;
		}
		try {
