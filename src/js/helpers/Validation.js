...
const Validation = {
	 ...
	 * @returns {boolean}
	 */
	isNullOrEmpty: (value) => value === null || value === undefined || (typeof value === "string" && value.trim() === ""),
	...
	/**
	 * @param {*} value
	 * @returns {boolean} Número finito válido
	 */
	isNumber: (value) => {
	 ...
	 * @returns {boolean}
	 */
	isValidEmail: (email, regex = config.validation.emailRegex) => typeof email === "string" && regex.test(email.trim()),
	 ...
	 * @returns {boolean}
	 */
	isValidPhone: (phone, regex = config.validation.phoneRegex) => typeof phone === "string" && regex.test(phone.trim()),
	 ...
	 * @returns {boolean}
	 */
	isValidFileSize: (file, maxMB) => file instanceof File && file.size <= maxMB * 1024 * 1024,
