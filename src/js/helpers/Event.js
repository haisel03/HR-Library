 ...
 * @module Event
 * @description
 * Bus de eventos pub/sub para Coffee Schools.
...
const Event = {
	...
	off(event, handler) {
		...
		const filtered = _bus.get(event).filter((h) => h !== handler && h._original !== handler);

		filtered.length ? _bus.set(event, filtered) : _bus.delete(event);
	},
