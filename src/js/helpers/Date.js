...
const DateHelper = {
	 ...
	 */
	isToday(value) {
		return DateHelper.startOfDay(value)?.getTime() === DateHelper.startOfDay(new Date())?.getTime();
	},
	...
	init(scope = document) {
		...
		(scope === document ? document : scope).querySelectorAll("[data-flatpickr]").forEach((el) => {
			// No reinicializar
